<script lang="ts">
    ...
    import { sopStatusLayers, statusMapsBaseLayers} from "../../../lib/utils/mapLayers";
	import { sopStatusUrl } from "$lib/utils/stores";
    ...
    onMount(()=> { 
        ...
        // create map
        map = new Map({
            layers: [ ...statusMapsBaseLayers,...sopStatusLayers],
            target: 'map',
            view: makeMapView(zoom),
            controls: []
        });
        addTitleH2('SOP Status', 'layerSwitcher')
        addLayerSwitcher(statusMapsBaseLayers, 'layerSwitcher', true , 'map');
        addLayerSwitcher(sopStatusLayers, 'layerSwitcher', true , 'map');

        

        sopStatusUrl.set(getLegendUrlFromLayer(sopStatusLayers[0]));
        let pointLayerSource = addPointLayer(map);
        const infoLayer = [sopStatusLayers[0]];
        getWMSInfoAndAddPointArray(map, sopStatusLayers, pointLayerSource);
    ...
    <div class="mapOverlay leftOverlay" id="panel" >
        ...
        {#if sopStatusUrl}
            <div class="legend" id="legend">
                ...
                <img id="legendImage" src= {$sopStatusUrl} alt="Legend">
            </div>
        {/if} 
