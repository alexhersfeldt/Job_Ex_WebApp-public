<script lang="ts">
    ...
    import { obliqueStatusLayers, statusMapsBaseLayers} from "../../../lib/utils/mapLayers";
    ...
    onMount(()=> { 
        ...
        // create map
        map = new Map({
            layers: [ ...statusMapsBaseLayers,...obliqueStatusLayers],
            target: 'map',
            view: makeMapView(zoom),
            controls: []
        });
        addTitleH2('Oblique Status', 'layerSwitcher')
        addLayerSwitcher(statusMapsBaseLayers, 'layerSwitcher', true , 'map');
        addLayerSwitcher(obliqueStatusLayers, 'layerSwitcher', true , 'map');

        

        obliqueStatusUrl.set(getLegendUrlFromLayer(obliqueStatusLayers[0]));
        let pointLayerSource = addPointLayer(map);
        const infoLayer = [obliqueStatusLayers[0]];
        getWMSInfoAndAddPointArray(map, obliqueStatusLayers, pointLayerSource);
